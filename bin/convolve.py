...
def convolve(image, psf, doPSF=True, edgeCheck=False):
    ...
    """
    datadim1, datadim2 = image.shape[0], image.shape[1]

    if datadim1 != datadim2:
        ddim = max(datadim1, datadim2)
        s = np.binary_repr(ddim - 1)
    ...
    if s.find('0') > 0:
        size = 2 ** len(s)
        if edgeCheck and size - ddim < 8:
            size *= 2
        boxd = np.zeros((size, size))
        r = size - datadim1
        r1, r2 = r // 2, r // 2
        if r % 2 == 1:
            r1 = r // 2 + 1
    ...
    #   go a bit quicker.
    b = np.fft.fftshift(np.fft.irfft2(b)).real
