...
class LensSample():
    ...
    def Generate_Lens_Pop(self, N, firstod=1, nsources=1, prunenonlenses=True, save=True):
        ...
        while M > 0:
            ...
            zs, ms, xs, ys, qs, ps, rs, mstar, mhalo = self.S.drawSourcePopulation(
                n * nsources, sourceplaneoverdensity=firstod, returnmasses=True)

            zl1 = zl * 1
            sigl1 = sigl * 1
            for i in range(nsources - 1):
                zl = np.concatenate((zl, zl1))
                sigl = np.concatenate((sigl, sigl1))
    ...
    def Pick_a_lens(self, i=None, dspl=False, tspl=False):
        ...
        for band in self.S.bands:
            ...
            if dspl or tspl:
                ...
                if tspl:
                    self.msi3[band] = self.ms3[band][i]

        preselection = self.apply_preselection(self.mli["i_SDSS"], self.zl[i])
        if dspl == False and tspl == False:
            return [self.mli, self.rli, self.ql[i], self.bl[i]], [self.msi, self.xs[i], self.yl[i], self.qs[i],
                                                                  self.ps[i], self.rs[i]], [
                       self.zl[i], self.zs[i]], preselection
        elif tspl == False:
            return [self.mli, self.rli, self.ql[i], self.bl[i]], [self.msi, self.xs[i], self.yl[i], self.qs[i],
                                                                  self.ps[i], self.rs[i]], [
                       self.bl2[i], self.msi2, self.xs2[i], self.yl2[i], self.qs2[i], self.ps2[i], self.rs2[i]], [
                       self.zl[i], self.zs[i], self.zs2[i], self.sigl[i], self.Mvs[i], self.r_phys[i]], preselection

        else:
            return [self.mli, self.rli, self.ql[i], self.bl[i]], [self.msi, self.xs[i], self.yl[i], self.qs[i],
                                                                   self.ps[i], self.rs[i]], [
                       self.bl2[i], self.msi2, self.xs2[i], self.yl2[i], self.qs2[i], self.ps2[i], self.rs2[i]], [
                       self.bl3[i], self.msi3, self.xs3[i], self.yl3[i], self.qs3[i], self.ps3[i], self.rs3[i]], [
                       self.zl[i], self.zs[i], self.zs2[i], self.zs3[i], self.sigl[i], self.Mvs[i], self.r_phys[i]], [
                       preselection]
