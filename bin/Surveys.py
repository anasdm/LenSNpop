...
class Survey():
    def  __init__(self,Name):
        ...
        if Name[:3]=="DES":
            ...
            twodg=pickle.load(open("../data/LensPop/2dpdfs/2dg_DES.pkl",'r'))
            twodr=pickle.load(open("../data/LensPop/2dpdfs/2dr_DES.pkl",'r'))
            twodi=pickle.load(open("../data/LensPop/2dpdfs/2di_DES.pkl",'r'))
            ...
            self.pixelsize=0.2#0.18
            self.side=60#111
            self.bands=['g','r','i']
            self.zeropoints=[30,30,30]
            self.zeroexposuretime=25
            self.skybrightnesses=[22.26,21.20,20.48]#[21.7,20.7,20.1]
            ...
            self.readnoise=(10/4.5)
            with open('../data/LensPop/2dpdfs/2dg_LSST.pkl', 'rb') as f:
                twodg = pickle.load(f, encoding='latin1')  # Use 'latin1' encoding for Python 2 pickle files
            with open('../data/LensPop/2dpdfs/2dr_LSST.pkl', 'rb') as f:
                twodr = pickle.load(f, encoding='latin1')  # Use 'latin1' encoding for Python 2 pickle files
            with open('../data/LensPop/2dpdfs/2di_LSST.pkl', 'rb') as f:
                twodi = pickle.load(f, encoding='latin1')  # Use 'latin1' encoding for Python 2 pickle files
            self.stochasticobservingdata=[twodg,twodr,twodi]
            if Name[-1]=="a":
                self.strategy="absolute"
                self.strategyx=10
            ...
            self.degrees_of_survey=150
            self.readnoise=(5)
            twodg=pickle.load(open("../data/LensPop/2dpdfs/2dg_CFHT.pkl",'r'))
            twodr=pickle.load(open("../data/LensPop/2dpdfs/2dr_CFHT.pkl",'r'))
            twodi=pickle.load(open("../data/LensPop/2dpdfs/2di_CFHT.pkl",'r'))
            self.stochasticobservingdata=[twodg,twodr,twodi]
            self.strategy="absolute"
            self.strategyx=10
            ...
            self.exposuretimes=[22400000000/3,22400000000/3,22400000000/3]
            self.gains=[1,1,1]
            ...
            self.bands=['g','r','i']
            self.zeropoints=[24.5,24.5,24.5]
            self.zeroexposuretime=0.25
            self.skybrightnesses=[220,220,220]
            self.exposuretimes=[2880,2880,2880]
            self.gains=[1,1,1]
            self.seeing=[.05,0.05,0.05]
            self.nexposures=1
            self.readnoise=(.005)
            twodr=numpy.array([[0.1,22],[0.1,22]])
            twodg=numpy.array([[0.1,22],[0.1,22]])
            twodi=numpy.array([[0.1,22],[0.1,22]])
            self.stochasticobservingdata=[twodg,twodr,twodi]
            self.degrees_of_survey=41253
        else:
