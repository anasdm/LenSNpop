...
import pickle
import indexTricks as iT
#import profiles,convolve,model
...
#===========================================================================
    def MakeModel(self, bands, images, mag_unlensed, mu, td, plot_SN = True):
        ...
        if plot_SN ==True:
            deltamag_SN = {'g_SDSS': 0.06, 'r_SDSS': 0.04, 'i_SDSS': 0.03}
            for band in bands:
                for i,im in enumerate(images):
                    self.galmodel[band][int((self.side/2)+im[0]/self.pixelsize),int((self.side/2)+im[1]/self.pixelsize)]+=mu[i] * 10**(-(mag_unlensed[band]-self.zeropoints[band]+td[i]*deltamag_SN[band])/2.5)
...
#===========================================================================
    ...
    def makeLens(self,stochastic=True,save=False,noisy=True,stochasticmode="MP",SOdraw=[],bands=[], images = [], mag_unlensed={}, mu=[],td=[], plot_SN = True, musthaveallbands=False,MakeModel=True):
        ...
        if MakeModel:
            self.MakeModel(bands, images, mag_unlensed, mu,td, plot_SN)
...
#===========================================================================
    ...
    def display(self, band="g_SDSS",bands=["g_SDSS","r_SDSS","i_SDSS"]):
        ...
        plt.ion()
        plt.figure(1)
        plt.imshow(self.makeColorLens(bands=bands),interpolation="none", origin = 'lower')
        plt.figure(2)
        import colorImage
        self.color = colorImage.ColorImage()#sigma-clipped single band residual
        plt.imshow(self.color.createModel(self.fakeResidual[0][band],self.fakeResidual[0][band],self.fakeResidual[0][band])[:,:,0],interpolation="none",origin='lower')

        plt.figure(3)
        plt.imshow(self.fakeResidual[0][band],interpolation="none",origin='lower')       
        
        try:
            self.fakeResidual[1]["RF"]
            plt.figure(4)
            plt.imshow(self.fakeResidual[1]["RF"],interpolation="none", origin = 'lower')
